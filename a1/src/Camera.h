...
Eigen::Vector2<Scalar> Camera::vectorToPixel(const Eigen::Vector3<Scalar> & rPCc) const
{
    ...
    if (distCoeffs.total() > 0) k1 = Scalar(distCoeffs.at<double>(0));
    if (distCoeffs.total() > 1) k2 = Scalar(distCoeffs.at<double>(1));
    if (distCoeffs.total() > 2) p1 = Scalar(distCoeffs.at<double>(2));
    if (distCoeffs.total() > 3) p2 = Scalar(distCoeffs.at<double>(3));
    if (distCoeffs.total() > 4) k3 = Scalar(distCoeffs.at<double>(4));
    if (distCoeffs.total() > 5) k4 = Scalar(distCoeffs.at<double>(5));
    if (distCoeffs.total() > 6) k5 = Scalar(distCoeffs.at<double>(6));
    if (distCoeffs.total() > 7) k6 = Scalar(distCoeffs.at<double>(7));
    if (distCoeffs.total() > 8) s1 = Scalar(distCoeffs.at<double>(8));
    if (distCoeffs.total() > 9) s2 = Scalar(distCoeffs.at<double>(9));
    if (distCoeffs.total() > 10) s3 = Scalar(distCoeffs.at<double>(10));
    if (distCoeffs.total() > 11) s4 = Scalar(distCoeffs.at<double>(11));
