...
SCENARIO("SystemSLAM::dynamics analytical Jacobian vs autodiff")
{
    GIVEN("A SLAM system with state and input vectors")
    {
        ...
        // Create input vector (6D for body velocities)
        Eigen::VectorXd u(6);
        u << 0.01, 0.02, 0.03,     // input velocities
             0.005, 0.01, 0.015;   // input angular velocities
        ...
        {
            Eigen::VectorX<dual> f_dual = SystemSLAM::dynamics(x_dual, u_dual);
