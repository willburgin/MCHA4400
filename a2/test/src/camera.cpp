...
SCENARIO("Lens distortion")
{
    GIVEN("A camera with lens distortion")
    {
        ...
        GIVEN("A test pixel rQOi")
        {
            ...
            rQOi << 0.25*cam.imageSize.width, 0.25*cam.imageSize.height;

            WHEN("rQbarOi = cam.undistort(rQOi)")
            {
                Eigen::Matrix<double, 2, Eigen::Dynamic> rQbarOi = cam.undistort(rQOi);
                ...
                AND_WHEN("rQOi = cam.distort(rQbarOi)")
                {
                    Eigen::Matrix<double, 2, Eigen::Dynamic> rQOi_actual = cam.distort(rQbarOi);
            ...
            WHEN("pQbarOi = cam.undistort(pQOi)")
            {
                Eigen::Matrix<double, 3, Eigen::Dynamic> pQOi(3, 1);
                pQOi << rQOi(0, 0), rQOi(1, 0), 1.0;
                Eigen::Matrix<double, 3, Eigen::Dynamic> pQbarOi = cam.undistort(pQOi);
                ...
                AND_WHEN("pQOi = cam.distort(pQbarOi)")
                {
                    Eigen::Matrix<double, 3, Eigen::Dynamic> pQOi_actual = cam.distort(pQbarOi);
