    ...
    , isFound(false)
{
    ...
    isFound = cv::findChessboardCorners(gray, chessboard.boardSize, detectedCorners, cv::CALIB_CB_ADAPTIVE_THRESH + 
...
void ChessboardImage::drawBox(const Chessboard & chessboard, const Camera & camera)
{
    ...
    std::vector<cv::Vec3d> baseCorners = 
    {
        {minX, minY, 0},      // Bottom-left
    ...
    auto drawCurvedLine = [&](const cv::Vec3d& P1, const cv::Vec3d& P2, const cv::Scalar& color, int thickness = 4, int numSegments = 1000) 
    {
...
void Camera::calibrate(ChessboardData & chessboardData)
{
    ...
    std::vector<std::vector<cv::Point3f>> rPNn_all_repeated(
        rQOi_all.size(),  // number of corners in one image
        rPNn_all          // 3D chessboard points
    );

    rms = cv::calibrateCamera(rPNn_all_repeated, rQOi_all, imageSize, cameraMatrix, distCoeffs, Thetacn_all, rNCc_all, flags);
    std::println(" done");
...
Eigen::Matrix<double, 2, Eigen::Dynamic> Camera::undistort(const Eigen::Matrix<double, 2, Eigen::Dynamic> & rQOi) const
{
    ...
    cv::undistortPoints(rQOi_cv, rQbarOi_cv, cameraMatrix, distCoeffs, I, cameraMatrix);
...
Eigen::Matrix<double, 2, Eigen::Dynamic> Camera::distort(const Eigen::Matrix<double, 2, Eigen::Dynamic> & rQbarOi) const
{
    ...
    {
        rPCc(0, i) = (rQbarOi(0, i) - cx) / fx;
        rPCc(1, i) = (rQbarOi(1, i) - cy) / fy;
        rPCc(2, i) = 1.0;
    }
    ...
    {
        // Use the templated vectorToPixel function
        Eigen::Vector3d rPCc_col = rPCc.col(i);
        rQOi.col(i) = vectorToPixel(rPCc_col);
    }
